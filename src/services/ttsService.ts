...
/**
 * TTS quan trọng: nếu volume media quá nhỏ thì tạm tăng rồi restore
 * (chỉ khi user không chỉnh volume trong lúc đọc).
 */
export async function speakImportantText(text: string, lang = 'vi-VN'): Promise<void> {
  ...
  try {
    ...
    console.warn('[Important TTS] Volume boost skipped:', error)
  }
  ...
  try {
    ...
    console.warn('[Important TTS] Speak failed:', error)
  }
